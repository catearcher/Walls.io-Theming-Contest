(function() {
  ...
  var template = '<li class="card"><a href="#" class="click-area"><div class="title"><span class="ellipsis"><%=external_name%></span></div><div class="description"><%=comment%></div><ul class="view-users-widget assignees"><li><img src="<%=external_image%>" height="24" width="24" class="image-wrapper-s bs-tooltip"></li><%if(post_image){%><li><img src="<%=post_image%>" height="24" width="24" class="image-wrapper-s bs-tooltip"></li><%}%></ul></a></li>';

  var queues = [];

  var createPostHtml = function(post) {
    return WallStream.tmpl(template, post);
  };

  var pruneOldPosts = function($stage) {
    $stage.find(".card").slice(10).remove();
  };

  var handleQueue = function(stageIndex) {
    var
    queue = queues[stageIndex],
    $stage = $(".stage").eq(stageIndex),
    postHtml = queue.shift();

    if (postHtml) {
      $stage.prepend(postHtml);
      pruneOldPosts($stage);
    }
  };

  var createWall = function(name, title) {
    var index = queues.length;

    queues.push([]);

    $(".q-stage-header-detail-content-wrapper").eq(index).text(title);

    new WallStreamCore({
      accessToken: "5f864451221b0e8d2ff61b3179ac1a3b5d4ac9e3",
      types: [name],
      onPost: function(post) {
        queues[index].push(createPostHtml(post));
      }
    });
  ...
  createWall("facebook", "Facebook");
  createWall("twitter", "Twitter");
  createWall("instagram", "Instagram");
  createWall("googleplus", "Google+");
  createWall("youtube", "YouTube");
